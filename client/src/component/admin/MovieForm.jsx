...
export default function MovieForm({ busy, btnTitle, initialState, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    const { tags, genres, casts, writers, director, poster } = movieInfo;
    ...
    const finalCast = casts.map((c) => ({
      actor: c.profile.id,
      roleAs: c.roleAs,
      leadActor: c.leadActor,
    }));
    finalMovieInfo.casts = JSON.stringify(finalCast);
  ...
  const updateCast = (castInfo) => {
    ...
    setMovieInfo({ ...movieInfo, casts: [...casts, castInfo] });
  };
  ...
  const handleCastRemove = (profileId) => {
    const { casts } = movieInfo;
    const newCast = casts.filter(({ profile }) => profile.id !== profileId);
    if (!newCast.length) hideCastModal();
    setMovieInfo({ ...movieInfo, casts: [...newCast] });
  };
    ...
    <>
      <div onSubmit={handleSubmit} className="flex space-x-3">
        <div className="w-[70%] space-y-5">
          ...
          <div>
            <div className="flex justify-between">
              <LabelWithBadge badge={casts.length}>
                Add Cast & Crew
              </LabelWithBadge>
              <ViewAllBtn onClick={displayCastModal} visible={casts.length}>
                View All
              </ViewAllBtn>
            </div>
